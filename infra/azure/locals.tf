...
locals {
  ...
  api_env_static = {
    ...
    # NEW env var the api-server needs to learn — see runbook §10.
    # Set unconditionally so the moment the code change lands, no infra change
    # is required to start consuming it.
